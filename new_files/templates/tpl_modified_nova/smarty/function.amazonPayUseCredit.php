...
function smarty_function_amazonPayUseCredit(){
    ...
    if($creditSelectionResult){
        $return = '<div style="clear:both;"><label><input type="checkbox" name="amazon_pay_use_credit" value="1" '.(!empty($_SESSION['cot_gv'])?'checked':'').' /> '.TEXT_AMAZON_PAY_USE_CREDIT.'</label></div>';
    }
    return $return;
}
